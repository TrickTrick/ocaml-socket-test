...
let create_timeout duration callback =
  Lwt_unix.sleep duration >>= fun () ->
  callback ()

let rec interval i callback =
...
let connect_to_server addr port () =
  ...
  let disc_timeout = create_timeout disc_duration (shutdown_process socket) in
  ...
  let keep_timeout = interval keep_duration cb in
